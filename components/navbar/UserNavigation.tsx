...
const UserNavigation = ({ currentUser }: Props) => {
  return (
    <AnimationWrapper transition={{ duration: 0.2 }} className="absolute right-0 z-50 rounded mt-1">
      <div className="bg-white dark:bg-[#343434] absolute right-0 border border-grey dark:border w-60 duration-200 rounded">
        ...
        <UserNavItem label="Dashboard" path="/blogs" icon={BsColumnsGap} />
        <UserNavItem label="Settings" path="/editprofile" icon={BsGear} />

        <hr className="border-grey" />
        <button className="p-4 w-full pl-8 py-4 link text-left" onClick={() => signOut()}>
